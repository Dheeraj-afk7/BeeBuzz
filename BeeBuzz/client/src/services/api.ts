...
// Admin-specific axios instance (uses its own token key)
...
const realAuthApi = {
  ...
  verifyDriver: (data: any) => api.post('/auth/verify-driver', data),
  payChallan: (data: any) => api.post('/auth/pay-challan', data),
  uploadDocument: (data: any) => api.post('/auth/document', data),
};
...
const realLoadApi = {
  ...
  verifyDelivery: (id: string) => api.post(`/loads/${id}/verify-pod`),
  rejectPod: (id: string, comment: string) => api.post(`/loads/${id}/reject-pod`, { comment }),
  aiPricing: (data: any) => api.post('/loads/ai-pricing', data)
};
...
const realBidApi = {
  ...
  accept: (bidId: string, data?: any) => api.put(`/bids/${bidId}/accept`, data),
...
// Admin API — uses adminAxios with separate token store
export const adminApi = {
  login: (data: any) => api.post('/auth/login', data), // reuse auth login, check role
  getStats: () => adminAxios.get('/admin/stats'),
  getUsers: (params?: any) => adminAxios.get('/admin/users', { params }),
  getUser: (id: string) => adminAxios.get(`/admin/users/${id}`),
  verifyUser: (id: string, data: any) => adminAxios.put(`/admin/users/${id}/verify`, data),
  getLoads: (params?: any) => adminAxios.get('/admin/loads', { params }),
  getPayments: (params?: any) => adminAxios.get('/admin/payments', { params }),
  getPendingDocuments: () => adminAxios.get('/admin/documents'),
};
