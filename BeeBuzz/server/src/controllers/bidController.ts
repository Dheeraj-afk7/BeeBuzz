...
// Helper to notify other drivers when they are outbid
async function checkAndNotifyOutbids(loadId: string, currentDriverId: string, amount: number) {
  try {
    const otherBids = await getAll(
      "SELECT b.*, u.name as driver_name FROM bids b LEFT JOIN users u ON b.driver_id = u.id WHERE b.load_id = ? AND b.driver_id != ? AND b.status = 'pending'",
      [loadId, currentDriverId]
    );
    for (const ob of otherBids) {
      if (ob.amount > amount) {
        ...
        await runQuery(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `, [notifId, ob.driver_id, 'Outbid Alert ⚠️', `You have been outbid on Load #${loadId.substring(0, 8)}. A competitive bid of ₹${amount} was placed!`, 'outbid', loadId]);

        sendToUser(ob.driver_id, {
          type: 'outbid',
          loadId,
          title: 'Outbid Alert ⚠️',
          message: `You have been outbid on Load #${loadId.substring(0, 8)}. A lower bid of ₹${amount} was placed!`,
          newAmount: amount,
          oldAmount: ob.amount
        });
      }
    ...
    console.error('Error checking and notifying outbids:', err);
  }
}
...
export const createBid = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    sendToUser(load.shipper_id, {
      type: 'new_bid',
      loadId,
      title: 'New Bid Received',
      message: `${driver.name} placed a bid of ₹${amount}`,
      amount
    });

    // Check if other drivers got outbid by this new bid
    await checkAndNotifyOutbids(loadId, req.user!.userId, amount);
...
export const acceptBid = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Process mock payment upfront from shipper
    const txnId = await paymentGateway.processPayment(load.price, req.body.paymentSource || 'mock_card');
    ...
    await runQuery(`
      INSERT INTO payments (id, load_id, shipper_id, driver_id, amount, platform_fee, net_amount, status, gateway_transaction_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'held', ?)
    `, [paymentId, load.id, load.shipper_id, bid.driver_id, load.price, platformFee, netAmount, txnId]);
...
export const updateBid = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Check if other drivers got outbid by this updated bid
    if (amount && amount < bid.amount) {
      await checkAndNotifyOutbids(bid.load_id, bid.driver_id, amount);
    }
